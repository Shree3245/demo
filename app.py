...
from db import newItemCreate,newPhysItemCreate,itemStockUpdate,itemOutStock,itemInsert5, notificationUnautItem, productInfo,itemStockUpdate2, userInfo, userInsert, itemInsert,itemInsert3, categories,envelope, catagoryCreate, itemInsert2,shopName
import re
import dateparser
...
def temp():
	incompleteVal ='False'
	multi =False
	if len(request.form.getlist('productName'))>1:
	...
	elif envelope == 'OtherEnve':
		envelope = request.form['envelopeName']
		if categories.objects(category=(request.form['envelopeName']).capitalize()).count()>0:
			pass
		else:
			catagoryCreate((request.form['otherCat']).capitalize())
	...
	if 1==1:
		if productInfo.objects(productName=productName).count()>1:
			...
			if productInfo.objects(SKU=SKU) == True:
				...
				notificationUnautItem.objects(itemName=productName).delete()
				productInfo.objects(productName=productName).delete()
				productInfo.objects(SKU=SKU).delete()
...
def editProduct(ItemName):
	if 'username' in session:
		for product in productInfo.objects( productName= ItemName):
			...
			return render_template('edit-product.html',product = product, currencyType = currencyType, cost = cost, envelope= envelopeItem,selectCat =cat,categories =categories.objects(),envelopes = envelope.objects(),SKU = product.SKU )
	return render_template('login.html')
		...
		if 'username' in session:
				...
				for i in tHistory.find():
					try:
						...
						if i["Sales Record Number"] not in x:
							x.append(i["Sales Record Number"])
					except:
						pass         
				...
				return render_template('daily-report.html',records = tHistory.find(),totalc = ("%.2f" % round(total,2)),quantity = quantity,transCount =len(x) )
		return render_template('login.html')
...
def upload_file():
	...
	for f in request.files.getlist('fileInput'):
		...
		if not in_db:
			...
			for iterA in range(len(x)):
				...
				if len(x[iterA])>10:
					...
					if dictIF != False:
						...
						if  productInfo.objects(SKU__contains=itemCode):
							skuLookup = productInfo.objects(SKU__contains=itemCode).first()
							...
							try:
								...
								try:
									skuLookup = productInfo.objects(SKU__contains=itemCode)
								except DoesNotExist:
									
									if len(temp)==0:
										newItemCreate(str(dictI['Custom Label']),dictI['Item Title'])
										newPhysItemCreate(str(dictI['Custom Label']),dictI['Item Title'],'NewSKU')														
						else:
							dictI['Status'] = 'Incomplete_SKU'
							temp = notificationUnautItem.objects(SKU = dictI['Custom Label'])
							try:
								skuLookup = productInfo.objects.get(SKU__contains=itemCode)
							except DoesNotExist:
			...
			for i in li:
				y = filter(lambda person: person['Custom Label']== '',li)
				for x in y:
					if x['Sales Record Number'] ==i['Sales Record Number']:
						i['Status']='Multi'
				...
				for x in rep:
					if [i['PayPal Transaction ID'],i['Sales Record Number'],i['Buyer Postcode']]==[x[0],x[1],x[2]]:
						i['Status'] ='Repeat'
...
@app.route('/delTran/<recordID>/<status>/<ItemNum>',methods=['POST', 'GET'])
def delTran(recordID,status,ItemNum):
	if 'username' in session:
		x = tHistory.find({'PayPal Transaction ID':(recordID),'Status':status,'Quantity':ItemNum})
		if x['Custom Label'] == '':
			tHistory.delete_many({'PayPal Transaction ID':(recordID),'Status':status,'Sales Record Number':x['Sales Record Number']})
		else:
			if x.count()>1:
				tHistory.delete_many({'PayPal Transaction ID':(recordID),'Status':status,'Quantity':ItemNum})
			else:
				tHistory.delete_one({'PayPal Transaction ID':(recordID),'Status':status,'Quantity':ItemNum})
		return redirect(url_for('dailyReports'))
	return redirect(url_for('index'))

@app.route('/multiDelTrans',methods=['POST', 'GET'])
def multiDelTrans():
	x = request.form.getlist('checkbox')
	for i in x:
		i = i.split()
		...
		if ItemNum == '':
			tHistory.delete_many({'PayPal Transaction ID':(recordID),'Status':status})

		z = tHistory.find({'PayPal Transaction ID':recordID ,'Status':status})
		for i in z:
			if i['Custom Label'] == ItemNum:

				tHistory.delete_many({'PayPal Transaction ID':(recordID),'Status':status,'Custom Label':ItemNum})
